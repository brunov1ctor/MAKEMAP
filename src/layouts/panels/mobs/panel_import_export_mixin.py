...
class ImportExportMixin:
    ...
    def _build_tools_panel(self) -> QWidget:
        ...
        # ── Page 1: read-only JSON/CSV export view — a live view of the
        # user's own mobs (see _build_mob_export), not an editable form
        # you "apply": there's nothing here to submit, just content to
    ...
    def _import_mob_dicts(self, data) -> int:
        ...
        if known_columns is None:
            logger.error("Import de mobs abortado: não foi possível confirmar as colunas válidas da tabela.")
            return 0
        imported = 0
        image_matches = 0
        for entry in data:
