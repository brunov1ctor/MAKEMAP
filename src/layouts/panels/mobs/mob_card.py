...
class _CoverImageLabel(QLabel):
    """A QLabel that shows the mob's whole image, scaled to fit its own
    ...
    def paintEvent(self, event):
        ...
        if self._radius:
            ...
            # MobCard._thumb), so nothing else clips its corners to match
...
def _set_icon_or_image(label: _CoverImageLabel, image_path: str, icon: str) -> float | None:
    ...
    None with no image — MobCard.set_data uses this to size THIS card's
...
def _rounded_zone_thumb(pixmap: QPixmap, size: int = _ZONE_BADGE_SIZE) -> QPixmap:
    ...
    is image-only (see MobCard.set_data), so this needs to read as a tiny
...
MIN_THUMB_H = 100
MAX_THUMB_H = 260
_CARD_MARGIN = 6  # matches MobCard's own layout.setContentsMargins below
...
class MobCard(QFrame):
    ...
    def __init__(self, mob_id: str, parent=None):
        ...
        meta_row.addWidget(self._element_label)
        ...
        # text only when the mob has no região tagged at all. Wrapped in
    ...
    def _build_badge_row(self) -> QHBoxLayout:
        ...
        # off the image entirely (see set_data/_element_label).
        top_row = QHBoxLayout()
        top_row.addStretch()
...
class MobListRow(QFrame):
    ...
    def set_data(self, name: str, level: int, category: str,
        ...
        # MobCard's grid tile — see that class) isn't used here: this
