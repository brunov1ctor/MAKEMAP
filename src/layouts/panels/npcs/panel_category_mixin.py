...
navigation, search, create/rename/delete. Mixed into NPCsPanel (see
panel.py) — operates on self.* attributes NPCsPanel owns; not meant to be
...
class CategoryExplorerMixin:
    ...
    def _build_left_column(self) -> QWidget:
        ...
        right-hand MobEditPanel slot, since a category isn't a npc and
    ...
    def _descendant_ids(self, folder_id: str) -> set[str]:
        ...
        while stack:
            current = stack.pop()
            for child in self._uow.npc_categories.get_children(current):
    ...
    def _refresh_explorer(self):
        ...
        path = self._uow.npc_categories.get_path(self._current_dir_id)
        ...
        children = [
            cat for cat in self._uow.npc_categories.get_children(self._current_dir_id)
        ...
        if children:
            for cat in children:
                ...
                row.set_count(sum(1 for m in self._npcs if m.get("category") in self._descendant_ids(cat["id"])))
    ...
    def _reload_categories(self) -> list[dict]:
        ...
        if not self._uow:
            return []
        all_categories = self._uow.npc_categories.get_all()
    ...
    def _on_edit_category_requested(self, key: str):
        if not self._uow:
            return
        cat = self._uow.npc_categories.get(key)
        if cat:
    ...
    def _on_category_editor_save(self, data: dict):
        ...
        if data["id"]:
            self._uow.npc_categories.update(data["id"], **fields)
        ...
        # color/category_image_border_color read from — the npc grid's
        ...
        # already trigger on every change) is what makes every npc card in
        # this category pick up the new color immediately, without the
        # user having to open and re-save each npc individually.
        self._apply_filters()
        self._left_stack.setCurrentIndex(0)
    ...
    def _reorder_categories(self, source_id: str, target_id: str):
        ...
        if not self._uow:
            return
        siblings = self._uow.npc_categories.get_children(self._current_dir_id)
        ...
        for i, cat_id in enumerate(ids):
            self._uow.npc_categories.update(cat_id, sort_order=i)
    ...
    def _on_delete_category(self, key: str) -> bool:
        ...
        self._uow.npc_categories.delete(key)
        if self._current_dir_id is not None and not self._uow.npc_categories.get(self._current_dir_id):
