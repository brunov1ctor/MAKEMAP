...
groups) and lives as page 1 of NPCsPanel's left-column QStackedWidget
...
NPCEditPanel slot (a category isn't an npc) and not a separate dialog/
...
from src.layouts.panels.mobs.edit_widgets import _DropImageButton  # generic widget, reused as-is (see npcs package docstring)
...
class _IconPicker(QWidget):
    ...
    this panel existed (or the DB default '🧙', npc_categories.icon), so a custom entry stays
...
class CategoryEditPanel(QFrame):
    """Create/edit panel for one npc_categories folder — the Mobs panel's
