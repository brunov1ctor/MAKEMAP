...
from src.layouts.panels.items.import_export_constants import (
    _ITEM_TEMPLATE_FIELDS, _ITEM_TEMPLATE_DOCS, _SKILL_TEMPLATE_FIELDS, _SKILL_TEMPLATE_DOCS,
)
...
class ItemsImportExportMixin:
    ...
    def _current_entity_label(self) -> str:
        return "Itens" if self._import_entity_mode == "item" else "Habilidades"

    def _current_catalog(self) -> list[dict]:
        return self._items if self._import_entity_mode == "item" else self._skills
    ...
    def _current_fields(self) -> dict:
        return _ITEM_TEMPLATE_FIELDS if self._import_entity_mode == "item" else _SKILL_TEMPLATE_FIELDS

    def _current_docs(self) -> list[tuple[str, str]]:
        return _ITEM_TEMPLATE_DOCS if self._import_entity_mode == "item" else _SKILL_TEMPLATE_DOCS

    def _current_categories(self) -> list[str]:
        return ITEM_CATEGORY_NAMES if self._import_entity_mode == "item" else SKILL_CATEGORIES
    ...
    def _current_import_fn(self):
        return self._import_item_records if self._import_entity_mode == "item" else self._import_skill_records
    ...
    def _build_tools_panel(self) -> QWidget:
        ...
        for mode, label in (("item", "Itens"), ("skill", "Habilidades")):
            btn = QToolButton()
            btn.setText(label)
    ...
    def _json_import_template(self) -> str:
        ...
        always resets back to — a single documented example row for the
        currently selected entity type, not the user's real data (Aplicar
        always CREATES new rows, so prefilling real data risks silently
        duplicating it)."""
        import json
        valid_categories = ", ".join(self._current_categories())
        valid_rarities = ", ".join(key for key, _label in rarity_options())
        doc_lines = "\n".join(f"// {key}: {doc}" for key, doc in self._current_docs())
        cat_line = f"// Categorias disponíveis: {valid_categories}"
        rarity_line = f"// Raridades disponíveis: {valid_rarities}"
    ...
    def _build_entity_export(self, fmt: str) -> str:
        ...
        rows = [{k: rec.get(k, self._current_fields()[k]) for k in fieldnames} for rec in self._current_catalog()]
        if fmt == "json":
            import json
    ...
    def _export_xlsx(self):
        ...
        for rec in self._current_catalog():
            ws.append([rec.get(k, self._current_fields()[k]) for k in fieldnames])
        wb.save(path)
        logger.info("%s exportado(s) para %s (%d linha(s))", self._current_entity_label(), path, len(self._current_catalog()))
