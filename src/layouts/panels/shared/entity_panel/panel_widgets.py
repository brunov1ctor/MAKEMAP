"""Standalone widget classes shared by MobsPanel and NPCsPanel — none
read/write panel internals, they're only instantiated and wired via
signals by whichever panel uses them, so they live here instead of being
duplicated once per entity (see mobs/panel_widgets.py and
npcs/panel_widgets.py, both now thin re-export shims of this module —
byte-for-byte identical between the two entities, so there was nothing
entity-specific left to parameterize)."""
...
def _rounded_thumbnail(path: str, size: int) -> QPixmap | None:
    ...
    (or NPCCard's) thumbnail, just baked into a flat QPixmap since QLabel
    has no paintEvent hook of its own to draw into."""
    src = QPixmap(path)
    if src.isNull():
        return None
...
class _SidebarRow(QFrame):
    ...
    def _on_rename_confirmed(self):
        ...
        if new_name and new_name != self._label.text():
            self._label.setText(new_name)  # optimistic — MobsPanel/NPCsPanel's own refresh follows right behind
...
# coupling to Mobs/NPCs internals) so Items e Habilidades can reuse it too
# — kept as an alias here so every existing `from
# src.layouts.panels.mobs.panel_widgets import _DropZone` (or npcs.
# panel_widgets) import keeps working unchanged.
...
class _SummaryCard(QFrame):
    ...
    splitter's, then this whole panel's effective minimum size. Once
