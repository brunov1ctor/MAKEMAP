"""CategoryEditPanel — full create/edit panel for a single category folder
(mob_categories or npc_categories). Replaces the old single inline text
field ("+ Nova categoria" in panel_category_mixin.py, icon always
hardcoded) with a real form: name, an icon grid, a card border color, an
uploaded image, and a separate border color around that image.

Shared by mobs/category_edit_panel.py and npcs/category_edit_panel.py —
byte-identical between the two entities except which emoji a brand-new
category defaults to (🐾 for mobs, 🧙 for npcs, see DEFAULT_ICON), so each
entity module subclasses CategoryEditPanel here and sets that one class
attribute instead of carrying its own ~720-line copy.

Mirrors RegionEditPanel's shell (glass paintEvent, CollapsibleSection
groups) and lives as page 1 of MobsPanel's/NPCsPanel's left-column
QStackedWidget (see panel_category_mixin.py's _build_left_column/self.
_left_stack) — swapped in over the CATEGORIAS sidebar itself, not the
right-hand edit-record slot (a category isn't a mob/npc) and not a
separate dialog/window. Every confirmation (missing name, delete) stays
inline in this panel too — no native QMessageBox popups, which read as a
...
from src.layouts.panels.mobs.edit_widgets import _DropImageButton  # generic widget, zero coupling to Mobs internals

# Curated set for the icon grid — the old fixed mob CATEGORY_DEFS icons
# (mobs/categories.py, now just seed data) plus a much wider spread of
# creature/monster/gear/element glyphs, so most category concepts (mob or
# npc) have a real pick instead of needing the free-text fallback. The grid
# scrolls (see _IconPicker) specifically so this list can stay long without
...
class _IconPicker(QWidget):
    ...
    def __init__(self, parent=None, default_icon: str = "🐾"):
        super().__init__(parent)
...
class CategoryEditPanel(QFrame):
    """Create/edit panel for one category folder (mob_categories or
    npc_categories) — the right-column 3rd page of whichever entity panel
    subclasses this. `load(None, parent_id)` opens in create mode inside
    `parent_id`; `load(category_dict, parent_id)` pre-fills every field for
    editing that category in place.

    DEFAULT_ICON is the one thing that genuinely differs between mobs and
    npcs (🐾 vs 🧙) — subclasses in mobs/category_edit_panel.py and
    npcs/category_edit_panel.py each override just this one class
    attribute."""
    ...
    def __init__(self, parent=None):
        ...
        self._icon_picker = _IconPicker(default_icon=self.DEFAULT_ICON)
    ...
    def load(self, category: dict | None, parent_id: str | None):
        ...
        self._icon_picker.set_icon((category or {}).get("icon") or self.DEFAULT_ICON)
