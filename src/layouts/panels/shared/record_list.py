"""_RecordCard / RecordListColumn — the card-list left column shared by the
DUNGEONS/CONSTRUÇÕES and QUESTS screens (dungeons/record_list.py and
quests/record_list.py used to each carry a full, byte-identical ~360-line
copy of this — the quests one even said so in its own docstring).

Everything that's genuinely identical between the two lives here. The only
real differences were: which `status_dot` a card colors its bolinha with
(dungeon vocabulary vs quest vocabulary — two different STATUS_COLORS
tables) and which emoji a card falls back to when it has no thumbnail image
(🏰 vs 📜). Both are exposed as class attributes (`STATUS_DOT`/
`DEFAULT_ICON` on _RecordCard, mirroring CategoryEditPanel.DEFAULT_ICON) so
each entity module subclasses _RecordCard/RecordListColumn here and sets
just those, instead of carrying its own copy of the whole card+list
mechanics (search, filters, drag&drop image assignment, arm-to-delete).
"""
...
from src.layouts.panels.items.constants import _INPUT_STYLE, _no_wheel, panel_frame_style, sub_header
...
class _RecordCard(QFrame):
    ...
    `STATUS_DOT` (a bolinha de status) e `DEFAULT_ICON` (fallback sem
    thumbnail) são as duas únicas coisas que variam por entidade — cada
    subclasse concreta (dungeons/record_list.py, quests/record_list.py)
    define só esses dois atributos."""

    STATUS_DOT = None  # subclasse define: staticmethod/function(status:str)->QLabel
    DEFAULT_ICON = "❔"
    ...
    def __init__(self, record: dict, parent=None):
        ...
        self._fallback_icon = record.get("icon") or self.DEFAULT_ICON
        ...
        if record.get("status"):
            row.addWidget(self.STATUS_DOT(record["status"]))
...
class RecordListColumn(QFrame):
    ...
    `CARD_CLASS` é a única coisa que uma subclasse concreta precisa trocar
    (para a sua própria subclasse de _RecordCard com STATUS_DOT/DEFAULT_ICON
    certos)."""
